...
class TurtleBot3:
    ...
    def move2goal(self):
        ...
        maze_map.map_1.r3_start=[x_initial,y_initial]
        maze_map.map_1.r3_goal = final_goal
        
        
        self.path =search.aStarSearch(current_maze,3,start_state,1,[],[],final_goal)
        ...
        while  i in range(len(self.path)-1):
         ...
         if i < (len(self.path)):   #edited on 15 december previousely i < (len(self.path)-1)
            ...
            while self.euclidean_distance(local_goal) >= distance_tolerance:
                ...
                if not collision_index_0 :
                    shifted_path=self.path0[1:]
                    collision_index_0 =[index for index, (item1, item2) in enumerate(zip(self.path,shifted_path)) if item1 == item2 and self.path.count(item1) == 1 and shifted_path.count(item2) == 1]
                    print("swapping condition collision index is ",collision_index_0)
