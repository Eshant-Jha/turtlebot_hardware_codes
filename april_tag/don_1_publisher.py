...
if __name__ == '__main__':
    rospy.init_node('tag_1_publisher')

    tfBuffer = tf2_ros.Buffer()
    listener = tf2_ros.TransformListener(tfBuffer)

    # Publisher for the apriltag_seven topic
    pub = rospy.Publisher('/apriltag_one', Odometry, queue_size=10)

    rate = rospy.Rate(50.0)
    while not rospy.is_shutdown():
        try:
            transformation = tfBuffer.lookup_transform('origin', 'one', rospy.Time())
            ...
            odometry_msg.child_frame_id = 'one'
